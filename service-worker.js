import { config as unoConfig } from "/unoAppTest/package_b24495cfe3f32501f30c25ce70f66c0e40c70436/uno-config.js";
...
if (unoConfig.environmentVariables["UNO_BOOTSTRAP_DEBUGGER_ENABLED"] !== "True") {
    ...
    self.addEventListener('install', function (e) {
        ...
        e.waitUntil(
            caches.open('d62eccd4-ff45-4abc-955a-592a0d5a761c').then(async function (cache) {
                ...
                try {
                    ...
                    const response = await fetch(`/unoAppTest/_framework/${unoConfig.dotnet_js_filename}`);
                    if (!response.ok) {
                    ...
                    const frameworkUris = [
                        ...
                        filename => `/unoAppTest/_framework/${filename}`
                    );
            ...
            caches.keys().then(function (cacheNames) {
                return Promise.all(
                    cacheNames.filter(function (cacheName) {
                        return cacheName !== 'd62eccd4-ff45-4abc-955a-592a0d5a761c';
